        ...
        public HttpResponseMessage GetAll(string langId)
        {
            try
            {
               ...
               // var currencyLogicTranslateogic = new CurrencyLogicTranslate();
                ...
                //Get Hotels
                List<HotelsModel> returnHotels = new List<HotelsModel>(hotelsTable.Rows.Count);
                foreach (DataRow dr in hotelsTable.Rows)
                {
                    ...
                    if (langId == Parameters.DefaultLang)
                    {
                        ...
                        returnHotels.Add(new HotelsModel
                        {
                            Id = (int)dr["Id"],
                            //DisplayValue = (string)dr["DisplayValue"],
                            //DisplayValueDesc = (string)dr["DisplayValueDesc"],
                    ...
                    else
                    {
                      //  var currencyTranslateInfo = currencyLogicTranslateogic.Get((int)dr["Currency"], langId);
                        ...
                        returnHotels.Add(new HotelsModel
                        {
                            ...
                            PriceStart = (int)dr["PriceStart"],
                            Rate = rateVlaue,
                           // CurrencyTitle = currencyTranslateInfo.Title
                        });
                ...
                //Get News
                List<NewsModel> returnNews = new List<NewsModel>(newsTable.Rows.Count);
                foreach (DataRow dr in newsTable.Rows)
                {
                    returnNews.Add(new NewsModel
                    {
                        Id = (int)dr["Id"],
                        //DisplayValue = (string)dr["DisplayValue"],
                        //DisplayValueDesc = (string)dr["DisplayValueDesc"],
                ...
                //Get Features
                List<FeaturesModel> returnFeatures = new List<FeaturesModel>(featuresTable.Rows.Count);
                foreach (DataRow dr in featuresTable.Rows)
                    returnFeatures.Add(new FeaturesModel
                    {
                        //DisplayValue = (string)dr["DisplayValue"],
                        //DisplayValueDesc = (string)dr["DisplayValueDesc"],
                        Icon = (string)dr["Icon"]
                ...
                //Get About
                var returnAbout = new AboutModel();
