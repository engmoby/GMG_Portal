    ...
    public class NewsController : ApiController
    {
        ...
        public HttpResponseMessage Save(News postedNews)
        {
            try
            {
                ...
                {
                    var newsLogic = new NewsLogic();
                    SystemParameter_News language = null;
                    if (postedNews.Id.Equals(0))
                    {
                        language = newsLogic.Insert(Mapper.Map<SystemParameter_News>(postedNews));
                    }
                    else
                    {
                        if (postedNews.IsDeleted)
                        {
                            language = newsLogic.Delete(Mapper.Map<SystemParameter_News>(postedNews));
                        }
                        else
                        {
                            language = newsLogic.Edit(Mapper.Map<SystemParameter_News>(postedNews));
                        }
                    }
